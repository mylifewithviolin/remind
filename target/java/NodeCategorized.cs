...
public partial class TargetJava
{
        ...
        private static int nodeCategorized(string source,List<SourceLines> sourcesLineList,List<string> importList,Dictionary<string, string> nameDictionary){
            int ret=1;//初期値 1:失敗
            ...
            for(int i=0;i<lineStrings.Length;i++){
                ...
                if(sourceLines.nodeKind==NodeKind.ND_STM_PACKAGE || sourceLines.nodeKind==NodeKind.ND_QAT_BEGIN){//引用開始の場合
                    sourceLineString=nameDictionaryReplase(nameDictionary,sourceLineString);
                    importList.Add(sourceLineString+";");//importリストに追加
                } 
            }
            ret=0;//ここまで進行したら成功
            return ret;
        ...
        private static NodeKind JugdeSourceLineNode(ref string sourceLineString){
            ...
            if(sourceLineString.IndexOf(SourceLineNodeKind.DefBeginNote)==0){
                ...
                if(sourceLineString.IndexOf(SourceLineNodeKind.PackageStNote)>=0){
                    nodeKind=NodeKind.ND_STM_PACKAGE;//パッケージ文
                }else{
